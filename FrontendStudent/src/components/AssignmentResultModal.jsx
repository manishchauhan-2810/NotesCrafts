// FrontendStudent/src/components/AssignmentResultModal.jsx
import React, { useState, useEffect, useRef } from 'react';
import { X, Award, TrendingUp, Loader } from 'lucide-react';
import { getAssignmentResult } from '../api/assignmentApi';

export default function AssignmentResultModal({ assignmentId, studentId, onClose }) {
  ...
  const fetchResult = async () => {
    try {
      setLoading(true);
      const response = await getAssignmentResult(assignmentId, studentId);
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-stretch justify-center z-50">
      ...
      <div className="bg-white w-full h-full max-w-none rounded-none flex flex-col">
        ...
        <div className="p-4 md:p-6 border-b border-gray-200 sticky top-0 z-30 bg-white flex items-center justify-between">
          <div>
            <h2 className="text-2xl font-bold text-gray-900">Assignment Result</h2>
            <p className="text-gray-600 mt-1 text-sm">{assignment.title}</p>
          </div>
          <button 
            onClick={onClose} 
